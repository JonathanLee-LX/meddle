...
function getPrompts(currentRulesText: string, contextFiles: RuleMergeContextFile[]): { system: string; user: string } {
  ...
  return {
    system: `你是 Easy Proxy 的路由规则整理助手。你的任务是合并和精简用户的路由规则，在不改变语义的前提下尽量减少规则数量。

请严格遵守以下要求：
1. 只输出最终规则文本，不要输出解释、标题、Markdown 代码块或额外说明。
2. 输出格式必须是 Easy Proxy 当前支持的 EPRC 文本格式，每行一条规则，target 固定在最后：
...
function getRuleGenerationPrompts(
  ...
  return {
    system: `你是 Easy Proxy 的路由规则生成助手。你的任务是根据用户的自然语言需求，生成可以直接使用的 EPRC 路由规则文本。
