...
const rateLimitedLogger = createRateLimitedLogger(console, {
    windowMs: Number(process.env.MEDDLE_LOG_RATE_LIMIT_WINDOW_MS) || 60000,
    maxPerWindow: Number(process.env.MEDDLE_LOG_RATE_LIMIT_MAX) || 20,
})
...
const proxyServer = http.createServer(async (req, res) => {
    ...
    if (requestTargetsProxy) {
        if (requestUrl.pathname === '/_meddle/ca.crt') {
            const rootCAPath = getRootCAPath()
            res.writeHead(200, {
                'Content-Type': 'application/x-x509-ca-cert',
                'Content-Disposition': 'attachment; filename="meddle-ca.crt"',
    ...
    if (!access.allowed) {
        ...
        if (access.statusCode === 407) headers['Proxy-Authenticate'] = 'Basic realm="Meddle"'
...
;(async () => {
    ...
    proxyServer.listen(port, remoteAccess.bindHost, () => {
        ...
        if (ctx.AUTO_OPEN) {
            ...
            const remotePort = process.env.MEDDLE_OPEN_CHROMEDEVTOOLS ? 9222 : undefined
            if (openBrowserWithProxy(proxyUrl, proxyAddr, ctx.meddleDir, remotePort)) {
        ...
        if (remoteAccess.enabled) {
            ...
            if (remoteAccess.token) {
                console.log(chalk.green('代理认证已启用，用户名: meddle'))
            } else {
...
proxyServer.on('connect', async (req, socket, head) => {
    ...
    if (!access.allowed) {
        const authenticate = access.statusCode === 407
            ? 'Proxy-Authenticate: Basic realm="Meddle"\r\n'
            : ''
