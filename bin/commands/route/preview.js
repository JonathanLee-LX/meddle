/**
 * ep route preview - Preview route target for a given URL
 */
...
async function run() {
  if (!urlArg) {
    ...
    output.info('Usage: ep route preview <url> [--file <rule-file>] [--json]')
    process.exit(1)
  }
  ...
  if (running) {
    ...
    try {
      ...
      if (!rulesText.trim()) {
        ...
        output.info('Use --file <name> to specify a rule file, or activate one with: ep route active set <file>')
        process.exit(1)
      }
