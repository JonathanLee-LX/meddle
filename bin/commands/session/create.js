/**
 * ep session create - Spawn a new isolated proxy session.
 *
 * Flow:
 *   1. Generate session id (label-timestamp) and allocate a free port
 *   2. Ensure the session's EP_HOME directory exists
 *   3. Spawn `node index.js` with EP_HOME + PORT env
 *   4. Wait for the child's HTTP API to become reachable
 *   5. Register in sessions.json (pid, port, epHome)
...
function parseArgs(argv) {
    const args = argv.slice(4) // after 'ep' 'session' 'create'
...
async function main() {
    ...
    if (port) {
        ...
        if (!port) throw new Error('no free port in [9000, 9999]; run `ep session prune` to clean orphaned records')
    }

    // 2. EP_HOME dir
    const epHome = sessionDir(id)
    fs.mkdirSync(epHome, { recursive: true })
    ...
    const childEnv = {
        ...
        // from ~/.ep/ca which the user already trusted when setting up
    ...
    createSession({ id, label: opts.name || '', port, pid: child.pid, epHome })
    ...
    if (opts.json) {
        ...
        console.log(`  ${chalk.gray('EP_HOME:')}   ${epHome}`)
        console.log(chalk.gray('\nUse this session with:'))
        console.log(`  ep --session ${id} route list`)
    }
}
