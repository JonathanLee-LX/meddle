...
if (sessions.length === 0) {
    ...
    console.log(chalk.gray('Create one with: ep session create --name <label>'))
    process.exit(0)
}
...
for (const s of sessions) {
    ...
    console.log(`    ${chalk.gray('epHome:')}  ${s.epHome}`)
...
if (orphaned.length > 0) {
    ...
    console.log(chalk.gray(`Run 'ep session prune' to clean up registry entries.`))
}
