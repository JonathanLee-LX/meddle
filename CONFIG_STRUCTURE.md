...
- Web 界面的“路由规则”页面
- `meddle route list/show/create/add/update/delete`
- `meddle route active` 和 `meddle route active set <file>`
...
```text
# 单条规则
app.example.com http://localhost:8000

# 多个 pattern 共用 target
app.example.com/console app.example.com/dev-server https://localhost:8000

# 排除部分路径
api.example.com !/api !/oauth !/internal http://localhost:5173

# 通配符
*.example.com 127.0.0.1:3000

# marker 路径重写
^https://app.example.com[/api/v2] https://localhost:13001
```
...
```text
https://app.example.com/api/v2/micro/app?a=1
```
...
2. 否则读取 `~/.meddle/mocks.json`。
3. 可通过设置页的“重新加载配置”立即重新读取。

### 系统设置

- 始终读取 `~/.meddle/settings.json`。
- 文件不存在时使用应用默认值。
- Web 设置页保存时会写入该文件。
...
```bash
ls -la ~/.meddle/
ls -la ~/.meddle/route-rules/
cat ~/.meddle/route-rules/默认规则.txt
cat ~/.meddle/mocks.json | jq .
cat ~/.meddle/settings.json | jq .
```
...
```bash
meddle route list
meddle route create local --content "api.example.com localhost:3000"
meddle route show local
meddle route active set local
meddle route preview "https://api.example.com/users"
```
...
```bash
tar -czf meddle-backup.tar.gz ~/.meddle/
```

仅备份主要配置：

```bash
tar -czf meddle-config-backup.tar.gz \
  ~/.meddle/route-rules \
  ~/.meddle/mocks.json \
  ~/.meddle/settings.json
```
...
```bash
tar -xzf meddle-backup.tar.gz -C ~/
```
