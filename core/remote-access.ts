...
export function buildRemoteAccessConfig(
    ...
    const enabled = argv.includes('--remote') || parseBoolean(env.MEDDLE_REMOTE) === true
    const interceptArg = argv.includes('--intercept-https')
        ? true
        : argv.includes('--no-intercept-https')
            ? false
            : undefined
    const interceptEnv = parseBoolean(env.MEDDLE_INTERCEPT_HTTPS)
    const token = readOption(argv, '--remote-token') ?? env.MEDDLE_REMOTE_TOKEN
...
function hasValidProxyAuthorization(header: string | string[] | undefined, token: string): boolean {
    ...
    try {
        ...
        return username === 'meddle' && timingSafeEqual(password, token)
    } catch (_) {
        return false
    }
}
...
export function createRemoteAccessInfo(
    ...
    const targets = port === null
        ...
        : addresses.map(address => ({
            ...
            certificateUrl: `http://${address}:${port}/_meddle/ca.crt`,
        }))
...
export function buildRemoteSetupHtml(
    ...
    const auth = requiresAuth
        ? '<li>代理认证用户名填写 <code>meddle</code>，密码填写启动时配置的口令。</li>'
        : '<li>当前未启用代理口令，仅允许局域网私有地址访问。请只在可信网络中使用。</li>'
    const https = interceptHttps
        ? '<li>访问 <a href="/_meddle/ca.crt">下载根证书</a>，在手机系统中安装并完全信任，才能查看 HTTPS 请求内容。</li>'
