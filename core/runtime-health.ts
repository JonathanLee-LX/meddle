...
export function parseWatchdogConfig(env: NodeJS.ProcessEnv = process.env): WatchdogConfig {
    return {
        enabled: env.EP_WATCHDOG !== '0',
        action: env.EP_WATCHDOG_ACTION === 'warn' ? 'warn' : 'exit',
        intervalMs: parseIntEnv(env.EP_WATCHDOG_INTERVAL_MS, 30000, 1000),
        minUptimeMs: parseIntEnv(env.EP_WATCHDOG_MIN_UPTIME_MS, 30000, 0),
        failureThreshold: parseIntEnv(env.EP_WATCHDOG_FAILURES, 3, 1),
        cpuPercent: parseNumberEnv(env.EP_WATCHDOG_CPU_PERCENT, 95, 1),
        rssBytes: parseNumberEnv(env.EP_WATCHDOG_RSS_MB, 1536, 1) * 1024 * 1024,
        connectionCount: parseIntEnv(env.EP_WATCHDOG_CONNECTIONS, 1000, 1),
        mitmServerCount: parseIntEnv(env.EP_WATCHDOG_MITM_SERVERS, 100, 1),
        fdCount: parseIntEnv(env.EP_WATCHDOG_FDS, 2048, 1),
        eventLoopDelayMs: parseNumberEnv(env.EP_WATCHDOG_EVENT_LOOP_MS, 1000, 1),
    }
}
