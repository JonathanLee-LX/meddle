...
Easy Proxy 插件系统支持三种运行模式，用于控制插件对请求处理的干预程度。
...
- **白名单**：可通过 `EP_PLUGIN_ON_HOSTS` 限制生效范围
- **适用场景**：
  - 插件验证完成后的正式上线
  - 需要插件实现复杂路由逻辑的场景
...
通过环境变量 `EP_PLUGIN_ON_HOSTS` 设置（逗号分隔）：

```bash
EP_PLUGIN_ON_HOSTS=api.example.com,test.example.com
```
...
| 变量 | 说明 | 默认值 |
|------|------|--------|
| `EP_PLUGIN_MODE` | 插件模式（off/shadow/on） | off |
| `EP_SHADOW_WARN_MIN_SAMPLES` | Shadow 模式警告最小样本数 | 200 |
| `EP_SHADOW_WARN_DIFF_RATE` | Shadow 模式警告差异率阈值 | 0.05 |
...
1. 设置 `EP_PLUGIN_MODE=shadow` 或通过 API 设置
...
2. 如果差异率 < 5%：
   - 设置白名单：`EP_PLUGIN_ON_HOSTS=api.example.com`
   - 观察白名单范围内的请求是否正常
   - 逐步扩大白名单范围
3. 如果差异率 < 1%：
   - 可以直接切换 `EP_PLUGIN_MODE=on`
   - 移除白名单限制
