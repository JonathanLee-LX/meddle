# RFC: Meddle 插件化架构重构方案（V1）

## 1. 背景

当前 Meddle 已具备较完整的核心能力（请求日志、规则配置、Mock、Replay 等），但能力主要以内聚在主进程代码中的方式演进。随着后续需求增加（动态 Mock、AI 请求分析、治理类能力），继续在单体流程上叠加将带来以下问题：
...
- 保持现有用户体验和配置兼容（多文件路由规则、Mock 配置、UI 操作路径）。
