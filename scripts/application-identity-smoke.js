...
async function main() {
  const lanAddress = process.env.EP_IDENTITY_SMOKE_HOST || getPrivateLanAddress()
  if (!lanAddress) {
    throw new Error('No private LAN IPv4 address found; set EP_IDENTITY_SMOKE_HOST to run the remote-device smoke test')
  }

  const proxyPort = await getFreePort()
  const originPort = await getFreePort()
  const secureOriginPort = await getFreePort()
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), 'easy-proxy-identity-smoke-'))
  const stdout = []
  const stderr = []
