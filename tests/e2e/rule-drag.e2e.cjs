...
async function waitForServer(baseUrl, child, output) {
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) {
      throw new Error(`Easy Proxy 提前退出 (${child.exitCode})\n${output.join('')}`)
    }
    try {
  ...
  throw new Error(`等待 Easy Proxy 启动超时\n${output.join('')}`)
}

async function main() {
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), 'easy-proxy-rule-drag-'))
